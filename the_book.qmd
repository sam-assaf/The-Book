...
```{r Ben Baldwin, echo = FALSE, fig.cap="Figure 3. Difference between “go for it” decisions from 2014 to 2020. (Ben Balwin)", dpi=300, out.width=600}
knitr::include_graphics('../../../MSBA-SA/The Book Project v_3/Final Submission/Visualizations/Ben Baldwin 4th Down.png')
# need to add footnote to ben baldwin
...
```{r WPA by play type and time remaining, echo = FALSE, fig.cap="Figure 5. This shows the distribution WPA by play type, all faceted by the score differential bins. An extremely interesting one to look at is when a possession team is down by 2 points, 1 point, or tied, the long tails on the distribution of field goal show how much making or missing that field goal can swing and change the win probability for the game (which is a play’s win probability added). Also note that the tails get longer as time left in the game decreases, again this is to be expected as plays can have more impact the less time there is left in a game. This is a major reason for the inclusion of not only time but the time difference ratio. Overall, this plot does a good job of showing how different plays can affect win probability depending on the score differential at the respective time of the game.", dpi=300}
...
```{r ELO and EPA, echo = FALSE, fig.cap="Figure 6. This plot supports the expected relationship between a team’s pregame ELO rating and their lagged pregame run EPA. As expected there is a clear positive relationship between the two. This visualization is intended to show that ELO is a fair metric to assess a team's strengths and weaknesses, assuming that EPA is a valid statistic for assessing a team as well.", dpi=300}
...
```{r fg cutoff, echo = FALSE, fig.cap="Figure 8. The cutoff of 38 yards is quite clearly seen as one can observe both the steep decline in number of field goals attempted and the decrease in the field goal probability based on historical data.", dpi=300}
